...
self.addEventListener('fetch', function (event) {
  ...
  // Respond to the event by opening the cache using the name CACHE_NAME
  event.respondWith(caches.open(CACHE_NAME).then((cache) => {
      //B8: If request in cache, return the cache version.
    return cache.match(event.request.url).then((cachedResponse) => {
      if (cachedResponse) {
        return cachedResponse;
      }

      //B8 Otherwise add it to cache and return network's response.
